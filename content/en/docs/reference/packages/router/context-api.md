...
Context objects are pooled and reused. **Never** store references to Context beyond the request handler. Check [Context Guide](/docs/guides/router/context/) for details.
{{% /alert %}}
...
For request binding, use the separate [binding package](/docs/reference/packages/binding/) or the [app package](/docs/guides/app/) for integrated binding with validation.
{{% /alert %}}
...
For tracing and metrics in your handlers, use the **app** package. The [app observability guide](/docs/guides/app/observability/) shows how to use `app.Context` methods such as `TraceID()`, `SpanID()`, `SetSpanAttribute()`, `AddSpanEvent()`, `RecordHistogram()`, `IncrementCounter()`, and `SetGauge()`.
...
- **Context Guide**: Learn about [Context usage](/docs/guides/router/context/)
- **Binding Package**: For full request binding, see [binding package](/docs/reference/packages/binding/)
