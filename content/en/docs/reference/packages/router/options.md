...
For tracing, metrics, and logging configuration, use the [app package](/docs/reference/packages/app/) which provides `WithObservability()`, `WithTracing()`, `WithMetrics()`, and `WithLogging()` options. These options configure the full observability stack and integrate with the router automatically.
{{% /alert %}}
...
- **App Package**: See [app observability options](/docs/reference/packages/app/)
