...
- Access log scope defaults to errors-only when not set via [WithAccessLogScope](/docs/reference/packages/app/observability-options/#withaccesslogscope). Reduces log volume.
...
Configure server timeouts and (optionally) transport. Default port is **8080** for HTTP and **8443** for TLS/mTLS; override with `WithPort`. For HTTPS or mTLS, use [WithTLS](/docs/reference/packages/app/options/#withtls) or [WithMTLS](/docs/reference/packages/app/options/#withmtls) at construction; see [Server](/docs/guides/app/server/) for examples.
